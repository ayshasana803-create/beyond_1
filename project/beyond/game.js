...
(() => {
  ...
  class CharacterRenderer {
    static draw(ctx, charType, state, animTime, x, y, scale = 1, options = {}) {
      ...
      const colors = {
        skin: isGirl ? '#fed7aa' : '#fde047',
        skinShade: isGirl ? '#fdba74' : '#facc15',
        hair: isGirl ? '#a855f7' : '#78350f', // Girl violet twin buns, Boy warm brunette
        hairAcc: isGirl ? '#ec4899' : '#0284c7', // Girl pink ribbon, Boy cyan cap
        shirt: isGirl ? '#f43f5e' : '#3b82f6',
        pants: isGirl ? '#ec4899' : '#1e3a8a',
        shoes: '#ffffff',
        shoeTrim: isGirl ? '#f43f5e' : '#ef4444',
      ...
      if (isGirl) {
        ...
        // Twin buns with bouncy offset
        const bunBounce = Math.sin(t * 10) * 2;
        // Left bun
        ctx.beginPath();
        ctx.arc(-18, -14 + bunBounce, 9, 0, Math.PI * 2);
        ctx.fill();
        // Right bun
        ctx.beginPath();
        ctx.arc(18, -14 - bunBounce, 9, 0, Math.PI * 2);
        ...
        ctx.beginPath();
        ctx.ellipse(-18, -6 + bunBounce, 4, 2.5, 0.3, 0, Math.PI * 2);
        ctx.ellipse(18, -6 - bunBounce, 4, 2.5, -0.3, 0, Math.PI * 2);
        ctx.fill();
      } else {
        // Boy hair & backwards baseball cap
        ctx.fillStyle = colors.hair;
        ctx.beginPath();
        ctx.arc(0, -5, 19, Math.PI * 0.9, 0.1);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.roundRect(-24, -8, 12, 5, [3, 0, 0, 3]);
        ctx.fill();
        // Cap button on top
  ...
  class GameEngine {
    ...
    triggerUselessActionStare() {
      ...
      const quotes = [
        ...
        "I'm tired.",
        "Was that necessary?",
        "Are you expecting an achievement?"
      ];
      const quote = quotes[Math.floor(Math.random() * quotes.length)];
      this.showSpeechBubble(quote);
      this.sound.speak(quote, this.selectedCharacter);
    }
        ...
        if (obs.type === 'bed') {
          if (!obs.resolved && this.charAnimState !== 'SLEEP') {
            ...
            if (!this.isGrounded && this.charY < obs.y - 5 && charWorldX > obs.x + 20 && charWorldX < obs.x + obs.width + 80) {
              obs.resolved = true;
              this.hideBedChoice();
              this.showToast('🦘 You jumped right over the cozy bed!');
            }
    ...
    handlePitFall(pit) {
      ...
      this.sound.speak('Whoa!', this.selectedCharacter);
    ...
    handleRockHit(rock) {
      ...
      this.showToast('🩹 Ouch! Stubbed toe on a rock. Limping for 5 seconds...');
    }
    ...
    handleBananaSlip(banana) {
      ...
      setTimeout(() => {
        ...
        this.currentSpeed = 0;
        this.showToast('🍌 Slipped on a banana peel! Rubbing sore back...');
      }, 600);

      // 3. Stand back up and continue
      setTimeout(() => {
        if (this.charAnimState === 'RUB_BACK') {
          this.charAnimState = 'WALK';
          this.currentSpeed = this.baseSpeed;
          this.sound.playJump();
        }
    ...
    startSleeping() {
      ...
      const charName = this.selectedCharacter === 'girl' ? 'Cute Girl' : 'Cute Boy';
    ...
    // =======================================================================
    // THE USELESS SHOP (Ironic purchases)
    // =======================================================================
    openShop() {
      this.gameState = 'SHOP';
      this.updateShopUI();
        ...
        if (this.stateTimer > 0) {
          ...
          if (this.stateTimer <= 0) {
            if (this.charAnimState === 'INJURED_WALK') {
              this.charAnimState = 'WALK';
              this.currentSpeed = this.baseSpeed;
              this.showToast('✨ Injured leg healed! Normal speed resumed.');
          ...
          if (this.charAnimState === 'PIT_FALL') {
            ...
            if (this.charY >= this.world.platformY + 80) {
              ...
              if (this.pitFailureCount % 2 === 1) {
                ...
                this.showToast('🪽 Fallen into the pit... Ascending to safety!');
                this.sound.speak('Flying out!', this.selectedCharacter);
              } else {
                this.charAnimState = 'PIT_CLIMB_RECOVERY';
                this.sound.playClimb();
                this.showToast('🪜 Fallen into the pit... Clambering back up!');
                this.sound.speak('Climbing up!', this.selectedCharacter);
              }
            }
          } else if (this.charAnimState === 'PIT_FLY_RECOVERY' || this.charAnimState === 'PIT_CLIMB_RECOVERY') {
            ...
            if (this.charY <= this.world.platformY) {
              ...
              this.currentSpeed = this.baseSpeed;
              this.particles.spawnDustPuff(this.charX, this.charY);
              this.showToast('✨ Recovered from pit! Journey continues...');
            }
          } else {
            // Normal jump gravity
            this.charVY += this.gravity * dt;
